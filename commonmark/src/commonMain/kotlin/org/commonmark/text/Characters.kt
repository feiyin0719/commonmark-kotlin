...
public object Characters {
    ...
    public fun find(c: Char, s: CharSequence, startIndex: Int): Int {
        val length = s.length
    ...
    public fun findLineBreak(s: CharSequence, startIndex: Int): Int {
        val length = s.length
        for (i in startIndex until length) {
            when (s[i]) {
                '\n', '\r' -> return i
    ...
    public fun isLetter(s: CharSequence, index: Int): Boolean {
        val codePoint = s.codePointAt(index)
        return codePoint.isLetterCodePoint()
    }

    public fun isSpaceOrTab(s: CharSequence, index: Int): Boolean {
        if (index < s.length) {
            when (s[index]) {
                ' ', '\t' -> return true
            }
    ...
    public fun skip(skip: Char, s: CharSequence, startIndex: Int, endIndex: Int): Int {
    ...
    public fun skipBackwards(skip: Char, s: CharSequence, startIndex: Int, lastIndex: Int): Int {
    ...
    public fun skipSpaceTab(s: CharSequence, startIndex: Int, endIndex: Int): Int {
        for (i in startIndex until endIndex) {
            when (s[i]) {
                ' ', '\t' -> continue
                else -> return i
    ...
    public fun skipSpaceTabBackwards(s: CharSequence, startIndex: Int, lastIndex: Int): Int {
        for (i in startIndex downTo lastIndex) {
            when (s[i]) {
                ' ', '\t' -> continue
                else -> return i
